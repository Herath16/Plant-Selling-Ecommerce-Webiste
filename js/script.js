...
(function () {
    ...
    /**
     * Handles the search submission from any page (usually index.html).
     * It redirects to products.html with the search query as a URL parameter.
     * @param {Event} event - The form submission event.
     */
    function handleSearchSubmission(event) {
        event.preventDefault(); // Stop the form from submitting normally (reloading)

        // Get the search input element that triggered the event (can be from index or products page)
        const currentSearchInput = event.target.querySelector('#search-bar');
        const query = currentSearchInput ? currentSearchInput.value.trim() : '';

        if (query) {
            // Redirect to the products page, passing the query in the URL
            window.location.href = `products.html?query=${encodeURIComponent(query)}`;
        } else {
            // Optional: Alert or prevent navigation if the search field is empty
            window.location.href = `products.html`; // Go to products page without filtering
        }
    }
    ...
    window.onscroll = () => {
        ...
        // --- FIX IMPLEMENTED HERE (Lines 370 & 373 in your old code) ---
        if (window.scrollY > 250) {
            if (header3) { // CHECK ADDED
                header3.classList.add('active');
            }
            if (scrollTop) { // CHECK ADDED
    ...
    if (document.getElementById('product-list')) {
        // 1. Check for a search query in the URL first
        const urlParams = new URLSearchParams(window.location.search);
        const searchQuery = urlParams.get('query');

        // 2. Call fetchProducts once, passing the query (will be null if not present)
