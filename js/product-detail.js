...
(function() { // <--- ADDED IIFE to localize variables and fix global 'currentUserId' conflict

    const REVIEWS_API_URL = 'http://localhost:3000/reviews';
    const PRODUCTS_API_URL = 'http://localhost:3000/products';
    const currentUserId = localStorage.getItem('userId'); // Now local to this function
    ...
    async function fetchAndRenderProductReviews(productId) {
        try {
            // FIX 1: Corrected fetch URL to use PRODUCTS_API_URL/productId/reviews
            // This should fix the GET 404 error
            const response = await fetch(`${PRODUCTS_API_URL}/${productId}/reviews`); 
    ...
    async function handleReviewSubmission(e) {
        ...
        try {
            // FIX 2: Corrected POST URL to use PRODUCTS_API_URL/review (assuming your backend uses this)
            // This should fix the POST 404 error
            const response = await fetch(`${PRODUCTS_API_URL}/review`, { 
                method: 'POST',
    ...
    async function handleRating(e) {
        ...
        try {
            // This endpoint for rating should be fine if it's handled by the /reviews router
